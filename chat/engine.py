...
def describe_error(exc: BaseException) -> str:
    """
    Map a raised exception to an operator-friendly Japanese message.

    Tool-level failures (e.g. mock_services returning 404) are already
    surfaced gracefully as MCP `isError` results and never raise here — this
    only handles failures to reach mcp_server or the Gemini API at all.
    """
    causes = exc.exceptions if isinstance(exc, ExceptionGroup) else [exc]
    for cause in causes:
        if isinstance(cause, (httpx.ConnectError, httpx.TimeoutException)):
            return "MCPサーバーに接続できませんでした。しばらくしてから再度お試しください。"
        if isinstance(cause, genai_errors.APIError):
            return "AIモデルとの通信でエラーが発生しました。しばらくしてから再度お試しください。"
