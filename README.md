...
- **Human-in-the-loop**: 再起動・スケール変更などの破壊的操作は、エージェントが提案した内容（対象サービス・変更内容・理由）を確認カードとして提示し、オペレーターが承認するまで実行されません。承認・実行された操作は監査ログに記録され、`/audit`ページで確認できます([ADR-0014](docs/adr/0014-streaming-tool-visibility-and-audit-log.md))。
- **リアルタイム可視化**: エージェントが調査中に呼んでいるMCPツール（メトリクス取得・ログ確認など）を、「考えています...」の間もリアルタイムに表示します。
...
- 状態はすべてインメモリで、サービス再起動時にリセットされます。監査ログ(`chat_api`)も同様です。
